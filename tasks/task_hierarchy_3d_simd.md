...
- [ ] Implement `Quaternion` rotation support for [ParentComponent](file:///d:/Codes/customized-godot/modules/ecs_core/entity_manager.h#85-86).
...
- [ ] Implement `SIMD_MatrixMultiply` using SSE/NEON intrinsics.
- [ ] Add `RestrictPointers` optimization for raw buffer access.
- [ ] Implement `BatchTransformUpdate` (processing 4 matrices in parallel).
- [ ] Optimize `DepthSorting` algorithm to minimize overhead.
...
- [ ] Implement [WorldTransform](file:///d:/Codes/customized-godot/modules/ecs_core/entity_manager.h#109-110) sync to `RenderingServer` via `VisualInstance3D`.
...
- [ ] [ADD] `HierarchySystem::validate_hierarchy_safety()`
